...
struct Cell {
    // ** Your Problem 3 code goes here! replace the following two lines **
     var cell = Cell(position: Position(row: 0, col: 0), state: CellState.empty)
}
/*:
 ## Problem 4:
...
/*
 It indicates no external label is used, no need to name the parameter in function call
         if function defined as : func addSomeFunctions(target: String)
         And then you have to call it as : addSomeFunctions(target: "My name")
         But if it defined: func addSomeFunctions(_ target: String)
         And then you have to call it as: addSomeFunctions("My name")
...
/*
 The return type of map2 is a two dimensional array of generic type T which in this case is a Cell Struct.
 */
/*:
...
/*
 It is an unknown type of return values, user can use it as any object, it is an optional value and T is a generic placeholder type that in our case will be a Cell Struct.
 */
...
/*
 It is a private offset value and it is used as a constant.
 It represents the offsets of the eight neighbours that the cell interact with (Position array with 8 items).       
...
/*
 The parameter internal name "cell" is implement in the function declaration and function calls to clearly indicate that we are trying to find neighbors of a "cell" struct and we use "cell" inside the function to reference this parameter.
 */
...
/*
 There is no T parameter here since reduce2() (reduce2 is exclusive to "Int"s that represent grid positions)) does not accept generic types as inputs or outputs, only integers as return type.
 */
...
/*
  It is approximately but not necessarily exactly 33 (there are 100 cells, the probability of alive cell is 1/3, so there should be about 33 alive cells) since we are using a random number generator to determine whether the state should be alive or empty.
 */
/*:
 ## Problem 14:
...
/*
 Its s struct value type of cell is Cell.
